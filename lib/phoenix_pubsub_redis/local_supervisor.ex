defmodule Phoenix.PubSub.Redis.LocalSupervisor do
  use Supervisor
  alias Phoenix.PubSub.Redis.Local
  ...
  def init([server, pool_size, dispatch_rules]) do
    ...
    true = :ets.insert(server, {:subscribe, Local, [server, pool_size]})
    true = :ets.insert(server, {:unsubscribe, Local, [server, pool_size]})
